...
def video(av_num):
    ...
    # 获取视频分P
    part = requests.get('https://api.bilibili.com/x/player/pagelist?aid={}&jsonp=jsonp'.format(av_num),headers=headers).text
    part_name = re.findall('"part":"(.*?)"',part)
    if part_name[0] == '':
        has_part = False
    else:
        has_part = True
    part_cid = re.findall('"cid":(.*?),',part)
    # 打印信息
    print('————作品信息————')
    print('标题:', title)
    ...
    if has_part:
        print('————分P信息————')
        for part_num in range(len(part_name)):
            print(str(part_num + 1) + 'P:',part_name[part_num])
    print('————作者信息————')
    print('作者:', owner)
    print('等级: LV',lv)
    print('签名:', tag)
    ...
    choice = input('是否展示评论？（Y/Enter）')
    if choice == 'Y' or choice == 'y':
        choice = input('请选择评论展示方式：按热度（H）/按时间（T）')
        sort = 0
        if choice == 'H' or choice == 'h': # 确定评论排序
            sort = '2'
        ...
        for x in range(len(like_details)):
            ...
            if y % 10 == 0: # 10个评论为一组显示
                if input('回车以查看下10个评论，输入exit退出评论:') == 'exit':
                    break
    choice = input('是否要下载视频？(Y/Enter)')
    if choice == 'Y' or choice == 'y':
        choice = input('选择你的下载方式:直接下载（D）/获取直链链接（G）')
        if choice == 'D' or choice == 'd':
            path = input('请输入下载路径:')
            # 使用you-get下载
            print('————下载视频和弹幕————')
            os.system('you-get ' + '-l ' + '-o ' + path + ' https://www.bilibili.com/video/av{}'.format(av_num))
            '''
            warning!
            下面这堆for循环可读性极低！！！
            '''
            # 重命名后缀为.cmt.xml的弹幕文件后缀为.xml，和视频文件统一
            for p_num in range(len(part_name)):
                print('ren "' + path + title + ' (p' + str(p_num+1) + '. ' + part_name[p_num] + ').cmt.xml" "' + title + ' (P' + str(p_num+1) + '. ' + part_name[p_num] + ').xml"')
                os.system('ren "' + path + title + ' (p' + str(p_num+1) + '. ' + part_name[p_num] + ').cmt.xml" "' + title + ' (P' + str(p_num+1) + '. ' + part_name[p_num] + ').xml"')
            # 记录目录下的所有字幕文件名称，转换时调用地址下的字母名称而不调用分P名，防止转换程序解码错误
            danmaku_file = []
            for file in os.listdir(path):
                if file.endswith('xml'):
                    danmaku_file.append(file)
            print('正在转换···如出现报错信息为语言读写错误，不影响转换')
            for p in range(len(part_name)):
                os.system('.\Danmaku2Ass\Kaedei.Danmu2Ass.exe ' + '"' + path + title + ' (P' + str(p + 1) + '. ' + part_name[p] + ').xml"')
            input('转换完成！')
            exit()
        elif download_way == 'G':
            if has_part:
                for cid_list in part_cid:
                    download_url = requests.get('http://www.xbeibeix.com/api/bilibiliapi.php?url=https://www.bilibili.com/&aid={}&cid={}'.format(av_num, cid_list))
                    target_url = re.findall('"url": "(.*?)"', download_url.text)[0]
                    print('——————————————————————————————————————————————')
                    print('视频:', target_url)
                    print('弹幕', 'https://comment.bilibili.com/{}.xml'.format(cid_list))
                input('获取成功')
                exit()
            # 获取视频cid
            ...
            target_url = re.findall('"url": "(.*?)"', download_url.text)[0]
            print('视频:',target_url)
            print('弹幕','https://comment.bilibili.com/{}.xml'.format(cid))
            input('获取成功')
...
def pessage(cv_num):
    # 获取网页
    text = requests.get('https://www.bilibili.com/read/cv{}'.format(cv_num)).text
    # 匹配正文
    char = re.findall('<p(.*?)</p>',text)[4:]
    print('————正文————')
    for center in range(len(char)):
        # 处理中间对齐的文字
        if '<strong>' in char[center]:
            char[center] = ' ' * 10 + re.findall('<strong>(.*?)</strong>',char[center])[0]
        if ';">' in char[center]:
            char[center] = char[center].split('>')[-1]
        # 去掉多余的描述符
        char[center] = char[center].replace('<br/>', '')
        char[center] = char[center].strip('>')
        char[center] = char[center].replace('&nbsp;', '')
    # 打印正文
    for write in char:
        print(write)
